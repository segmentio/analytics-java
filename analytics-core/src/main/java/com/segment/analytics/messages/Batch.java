...
public abstract class Batch {
  ...
  public static Batch create(Map<String, ?> context, List<Message> batch) {
	Message sentAtNull = batch.stream()
							.filter(message -> message.sentAt() != null )
							.findAny()
							.orElse(null);
			
    return new AutoValue_Batch(batch, sentAtNull == null ? new Date() : sentAtNull.sentAt(), context, SEQUENCE_GENERATOR.incrementAndGet());
  }
